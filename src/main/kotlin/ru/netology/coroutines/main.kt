...
fun main() {
    ...
    PostService.getAuthors().forEach { println(it) }
}
...
suspend fun getAuthor(client: OkHttpClient, id: Long, mutex: Mutex): Author =
    mutex.withLock {
        PostService.getAuthors().find { it.id == id } ?: makeRequest(
            "$BASE_URL/api/slow/authors/$id",
            client,
            object : TypeToken<Author>() {}).let {
            PostService.addAuthors(it)
            return@let it
        }
    }